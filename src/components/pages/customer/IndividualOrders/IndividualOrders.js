...
const IndividualOrders = (props) => {
    ...
    useEffect(() => {
        const getOrders = async () => {
            await axios.post(`https://custard-cake-backend-api.herokuapp.com/individual-orders`,email)
                .then(res => {
                    setOrders(res.data);
                });
        };
        ...
        <Container>
                ...
                <div className="modal-dialog  " role="document">
                    <div className="modal-content bg-color-main">
                        ...
                        <div className="modal-body bgColorMain">
                            ...
                            <p className="text-white">Item Name : {selectedItem?.cakeName}</p>
            ...
            <div className="modal fade " id="deleteModalCenter" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div className="modal-dialog  " role="document">
                    <div className="modal-content bg-color-main">
                        <div className="modal-header bgColorMain">
                            <h5 className="modal-title text-color-second" id="exampleModalLongTitle ">Are You sure Want To Cancel order !!</h5>
                            <div type="button" className="close" data-dismiss="modal" aria-label="Close">
                                <span className="cancel-x" aria-hidden="true">&times;</span>
                            </div>
                        </div>
                        <div className="modal-body bgColorMain">
                            <p className="text-white">Name : {selectedItem?.cakeName}</p>
                ...
                <tbody>
                    ...
                    {orders?.map((ele, index) => (
                        <tr key={ele?._id}>
                            ...
                            <td class="bg-secondary " data-toggle="modal" >{ele?.status} <i className="fas fa-edit text-color-second"></i></td>

                            <td class="bg-secondary cursorPointer" data-toggle="modal" data-target="#deleteModalCenter" onClick={() => (handleWantToDelete(ele))}>Cancel</td>
                        </tr>
                    ))}
                </tbody>
            </table>
